...
public interface TINRepository extends PostgresTableRepository<TIN> {
    ...
    @Query(value = "select ST_AsEWKT(ST_MakePolygon(ST_ExteriorRing((ST_Dump(ST_Union(geometry))).geom))) from #{#entityName} where id = ?#{[0]}", nativeQuery = true)
    String getExteriorRing(UUID featureId);
}
